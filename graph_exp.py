...
while True:
    ...
    for minute in weather:
        ...
        # Log date (for show all data)
        if showAllData and date not in dates:
            dates.append(date)

    # Begin plotting
    plt.xlabel("Time" if not showAllData else "Days")
    plt.ylabel(datakey_label)
    if xlabel and not showAllData:
        plt.xticks(x, xlabel)

    elif xlabel and showAllData:
        intv = round(len(x) / len(dates))
        cint, ticks = min(x), []
        for _ in dates:
            cint += intv
            ticks.append(cint)
